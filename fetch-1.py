...
def scrape_yellowpages(search_query: str) -> list:
    ...
    for card in cards[:20]:
        try:
            ...
            businesses.append({
                ...
                "email": "",
                "website": website,
                "google_maps_url": f"https://www.google.com/maps/search/{quote_plus(name + ' San Antonio TX')}",
                ...
                "monthly_retainer_value": cat_info.get("monthly_value", 1500),
                "urgency": cat_info.get("urgency", "low"),
                "ads_presence": False,
                "source": "YellowPages",
            })
    ...
    log.info(f"[YellowPages] Found {len(businesses)} for '{search_query}'")
    return businesses
...
async def main():
    ...
    log.info("Based on Seven Figure Agency Roadmap by Josh Nelson")
    log.info(f"Timestamp: {datetime.now(timezone.utc).isoformat()}")
    log.info(f"Targeting {len(SEARCH_CATEGORIES)} niche categories")
    log.info("=" * 60)

    all_businesses = []

    for search_query in SEARCH_CATEGORIES.keys():
        try:
            results = scrape_yellowpages(search_query)
            all_businesses.extend(results)
            time.sleep(1.5)
        except Exception as e:
            log.error(f"Scrape error for '{search_query}': {e}")
    ...
    for i, biz in enumerate(unique):
        ...
        try:
            ...
            # Find real email from website (no hallucinating)
            if biz.get("website") and not biz.get("email"):
                log.info(f"  → Looking for email on {biz.get('website')}")
                email = find_email_from_website(biz.get("website", ""))
                if email:
                    biz["email"] = email
            ...
            # Sales notes based on Josh Nelson's framework
            biz["notes"] = generate_notes(biz, flags, audit)
    ...
    # Separate hot/warm/cold based on Josh's thresholds
    hot = [b for b in qualified if b.get("score", 0) >= 70]
    warm = [b for b in qualified if 50 <= b.get("score", 0) < 70]
    has_email = [b for b in qualified if b.get("email")]

    log.info(f"Qualified leads: {len(qualified)}")
    log.info(f"Hot leads (70+): {len(hot)}")
    log.info(f"Warm leads (50-69): {len(warm)}")
    log.info(f"Leads with real email found: {len(has_email)}")
    ...
    payload = {
        ...
        "hot_leads": len(hot),
        "warm_leads": len(warm),
        "leads_with_email": len(has_email),
        "leads": [
            {
                ...
                "monthly_retainer_value": b.get("monthly_retainer_value", 1500),
    ...
    # GHL CSV Export with enhanced fields
    csv_path = EXPORTS_DIR / "ghl.csv"
    ghl_fields = [
        "Business Name", "Contact Name", "Phone", "Email", "Website",
        "City", "State", "Zip", "Lead Score", "Category",
        "Monthly Retainer Value", "Rating", "Review Count",
        "Has Email", "Urgency", "Flags", "Pain Points / Notes", "Source URL",
    ]
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for b in payload["leads"]:
            writer.writerow({
                ...
                "Lead Score": b["score"],
                "Category": b["category"],
                "Monthly Retainer Value": f"${b.get('monthly_retainer_value', 1500):,}",
                "Rating": b["rating"],
                "Review Count": b["review_count"],
                "Has Email": "Yes" if b["email"] else "No",
                "Urgency": b.get("urgency", "").upper(),
                "Flags": "; ".join(b["flags"]),
                "Pain Points / Notes": b["notes"],
                "Source URL": b.get("google_maps_url", ""),
            })
    log.info(f"Saved GHL CSV: {csv_path}")

    log.info("=" * 60)
    log.info(f"DONE — {len(qualified)} qualified leads saved.")
    log.info(f"HOT leads ready to call: {len(hot)}")
    log.info(f"Leads with real emails: {len(has_email)}")
    log.info("=" * 60)
